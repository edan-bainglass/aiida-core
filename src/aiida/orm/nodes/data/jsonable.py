...
class JsonableData(Data):
    ...
    def _get_object(self) -> JsonSerializableProtocol:
        ...
        try:
            ...
            raise ImportError(f'the objects module `{module_name}` does not contain the class `{class_name}`.') from exc
    ...
    def _validate(self) -> None:
        ...
        try:
            ...
            raise ValidationError(f"module '{module_name}' could not be imported.") from exc

        try:
            cls = getattr(module, class_name)
        except AttributeError as exc:
            raise ValidationError(f"module '{module_name}' does not contain class '{class_name}'.") from exc

        if not callable(getattr(cls, 'from_dict', None)):
            raise ValidationError(f"class '{module_name}.{class_name}' does not define a callable 'from_dict' method.")
