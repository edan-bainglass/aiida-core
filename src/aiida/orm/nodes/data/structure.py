...
class KindsAdapter(ModelAdapter[list[Kind], list[dict[str, t.Any]], qb_fields.QbArrayField]):
    """Adapter to convert between a list of Kind instances and a list of dictionaries representing them."""

    @classmethod
    def to_model(cls, value: list[Kind], *, context: t.Any = None) -> list[dict[str, t.Any]]:
        return [kind.get_raw() for kind in value]

    @classmethod
    def to_orm(cls, value: list[dict[str, t.Any]], *, context: t.Any = None) -> list[Kind]:
        return [Kind(raw=kind) for kind in value]
...
class SitesAdapter(ModelAdapter[list[Site], list[dict[str, t.Any]], qb_fields.QbArrayField]):
    """Adapter to convert between a list of Site instances and a list of dictionaries representing them."""

    @classmethod
    def to_model(cls, value: list[Site], *, context: t.Any = None) -> list[dict[str, t.Any]]:
        return [site.get_raw() for site in value]

    @classmethod
    def to_orm(cls, value: list[dict[str, t.Any]], *, context: t.Any = None) -> list[Site]:
        return [Site(raw=site) for site in value]
...
class StructureData(Data):
    ...
    def from_components(
        cls,
        cell: t.Iterable[t.Iterable[float]] = _DEFAULT_CELL,
        pbc: bool | t.Iterable[bool] = (True, True, True),
        kinds: list[Kind | dict[str, t.Any]] | None = None,
        sites: list[Site | dict[str, t.Any]] | None = None,
        **kwargs: t.Any,
    ) -> Self:
        """Construct a structure from its cell, periodicity, kinds and sites."""
        instance = cls(**kwargs)
        instance.set_cell(cell)
        instance.set_pbc(pbc)
    ...
    def from_pymatgen_structure(cls, structure: t.Any, **kwargs: t.Any) -> Self:
        ...
        instance = cls(**kwargs)
        instance.set_pymatgen_structure(structure)
        return instance
    ...
    @cell.setter
    def cell(self, value: t.Iterable[t.Iterable[float]]) -> None:
        self.set_cell(value)

    @attribute(
        model_adapter=KindsAdapter(),
        model_field_info=pdt.fields.FieldInfo(default_factory=list),
        model_metadata=(
            pdt.BeforeValidator(
                lambda value: [kind.get_raw() if isinstance(kind, Kind) else kind for kind in value],
            ),
        ),
    )
    ...
    @attribute(
        model_adapter=SitesAdapter(),
        model_field_info=pdt.fields.FieldInfo(default_factory=list),
        model_metadata=(
            pdt.BeforeValidator(
                lambda value: [site.get_raw() if isinstance(site, Site) else site for site in value],
            ),
        ),
    )
    ...
    def pbc(self) -> tuple[bool, bool, bool]:
        ...
        """
        return self.pbc1, self.pbc2, self.pbc3
    ...
    def set_ase(self, aseatoms: t.Any) -> None:
    ...
    def set_pymatgen(self, obj: t.Any, **kwargs: t.Any) -> None:
    ...
    def set_pymatgen_molecule(self, mol: t.Any, margin: float = 5) -> None:
    ...
    def set_cell(self, value: t.Iterable[t.Iterable[float]]) -> None:
        """Set the cell."""
        ...
        self.base.attributes.set('cell', _get_valid_cell(value))
    ...
    def reset_sites_positions(self, new_positions, conserve_particle=True):
        ...
        for i in range(n_sites):
            ...
            new_site = Site(site=self.sites[i])
    ...
    def set_pbc(self, value: bool | t.Iterable[bool]) -> None:
        """Set the periodic boundary conditions."""
        from aiida.common.exceptions import ModificationNotAllowed
    ...
    def get_site_kindnames(self) -> list[str]:
    ...
    def get_ase(self) -> t.Any:
        """Get the ASE object.
        Requires to be able to import ase.
        """
        return self._get_object_ase()

    def get_pymatgen(self, **kwargs: t.Any) -> t.Any:
    ...
    def get_pymatgen_structure(self, **kwargs: t.Any) -> t.Any:
    ...
    def append_kind(self, kind: Kind) -> None:
        ...
        self._internal_kind_tags[len(raw_kinds) - 1] = kind._internal_tag

    def append_site(self, site: Site) -> None:
    ...
    def append_atom(self, **kwargs):
        ...
        if 'name' not in kwargs:
            ...
            for idx, existing_kind in enumerate(_kinds):
                if self._internal_kind_tags is not None:
                    try:
        ...
        self.append_site(Site(kind_name=kind.name, position=position))

    def clear_kinds(self) -> None:
    ...
    def get_kind(self, kind_name: str) -> Kind:
        """Return the kind object associated with the given kind name."""
        if self.is_stored:
            try:
                kinds_dict = self._kinds_cache
            except AttributeError:
                self._kinds_cache = {kind.name: kind for kind in self.kinds}
                kinds_dict = self._kinds_cache
        else:
            kinds_dict = {kind.name: kind for kind in self.kinds}
    ...
    def get_kind_names(self) -> list[str]:
        ...
        """
        return [kind.name for kind in self.kinds]
    ...
    def _validate(self) -> None:
        ...
        counts = Counter(kind.name for kind in kinds)
        for name, count in counts.items():
            if count != 1:
                raise ValidationError(f"Kind with name '{name}' appears {count} times instead of only one")
        ...
        for site in sites:
            if site.kind_name not in kind_names:
                raise ValidationError(f'A site has kind {site.kind_name}, but no specie with that name exists')

        kinds_without_sites = kind_names - {site.kind_name for site in sites}
    ...
    def _prepare_cif(self, main_file_name=''):
        ...
        cif = CifData.from_ase(self.get_ase())
    ...
    def _get_object_pymatgen_structure(self, **kwargs):
        ...
        if kwargs.pop('add_spin', False) and any(n.endswith('1') or n.endswith('2') for n in self.get_kind_names()):
            ...
            for site in self.sites:
                ...
                species.append(Specie(kind.symbols[0], oxidation_state, spin=spin))
        else:
            for site in self.sites:
                kind = self.get_kind(site.kind_name)
