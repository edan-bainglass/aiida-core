...
class EntryPointData(Data):
    ...
    def from_entry_point(cls, entry_point: EntryPoint, **kwargs: t.Any) -> Self:
        ...
        if not isinstance(entry_point, EntryPoint):
            raise TypeError(f'entry_point should be of type `EntryPoint`, got: {type(entry_point)}')
    ...
    def _validate(self) -> None:
        ...
        if self.module != entry_point.module:
            raise exceptions.ValidationError(
                f'Stored entry point module `{self.module}` does not match `{entry_point.module}`.'
            )

        if self.attr != entry_point.attr:
            raise exceptions.ValidationError(
                f'Stored entry point attribute `{self.attr}` does not match `{entry_point.attr}`.'
            )
