...
class EnumData(Data, t.Generic[_EnumT]):
    ...
    def get_member(self) -> _EnumT:  # type: ignore[type-var]
    ...
    def _validate(self) -> None:
        ...
        try:
            ...
            raise ValidationError(f"The stored value '{value}' is not a valid value for the enum '{enum}'.") from exc

        if member.name != name:
            raise ValidationError(
                f"Attribute 'name' says '{name}' but '{member.name}' was reconstructed from the stored value."
            )
